...
(function () {
  ...
  function donationCents() {
    return state.isCustom ? state.customCents : PRESETS[state.presetIdx];
  }
  ...
  document.getElementById('park-form').addEventListener('submit', async function(e) {
    ...
    try {
      ...
      var res = await fetch('/api/checkout', {
        ...
        body: JSON.stringify({ licensePlate: plate, durationHours: dur.hours, donationCents: donationCents() }),
      });
...
export function ParkPage() {
  return (
    <Layout title="Pay to Park | ParkGive">
